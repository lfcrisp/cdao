...
<center>
	<div style="padding:10px 60px 20px 60px">
	    <form id="ff_insert_user" method="post">
	    	<table cellpadding="5">
	    		<tr>
	    			<td>真实姓名:</td>
	    		...
	    		<tr>
	    			<td>部门:</td>
	    			<td>
	    				<input class="easyui-textbox" name="deptid" id="t1" data-options="buttonIcon:'icon-large-chart',onClickButton:insert_dept">
	    ...
	    <div style="text-align:center;padding:5px">
	    	<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitForm_insert_user()">保存</a>
	...
	<script>
		...
		function setDeptid(treeNode){
			...
			$("#t1").textbox("setValue",treeNode.id);
			$("#t1").textbox("setText",treeNode.name);
		...
		function submitForm_insert_user(){
			var url="<%=request.getContextPath()%>/user/insertUser.action";
			$.post(url,$("#ff_insert_user").serialize(),function(flag){
				if(flag){
					$.messager.show({
						title:'温馨提示',
						msg:'数据添加成功',
